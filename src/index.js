...
for (let i = 0; i < geometry.attributes.position.array.length; i = i+3) {
  const obj = new THREE.Mesh( boxer, materials[Math.round(feet.map(fxrand(), 0, 1, 0, 9))]);
  ...
  obj.rotation.x = geometry.attributes.position.array[i]/2;
  obj.rotation.y = geometry.attributes.position.array[i+1]/2;
  obj.rotation.z = geometry.attributes.position.array[i+2]/2;
  scene.add(obj);
}
